...
def water_effect(image):
    water_image = cv2.stylization(image, sigma_s = 199, sigma_r = 0.1)
    status, output = cv2.imencode('.PNG', water_image)

    return output

def invert(image):
    inverted_image = cv2.bitwise_not(image)
    status, output = cv2.imencode('.PNG', inverted_image)

    return output

def hdr(image):
    hdr_image = cv2.detailEnhance(image, sigma_s = 12, sigma_r = 0.15)
    status, output = cv2.imencode('.PNG', hdr_image)
